...
export function HeroGallery({ heroes }: HeroGalleryProps) {
  ...
  const filteredHeroes = useMemo(() => {
    return heroes.filter((hero) => {
      // FIX 1: Explicitly ignore the "none" placeholder
                ...
                >
                  {ROLES.map((role) => (
                    <button
                      ...
                      className={`w-full text-left px-5 py-4 text-sm font-bold uppercase tracking-widest ${selectedRole === role ? "bg-white text-black" : "text-zinc-400"}`}
                    >
                      {role}
                    </button>
                  ))}
          ...
          <div className="hidden md:flex flex-wrap gap-2">
            {ROLES.map((role) => (
              <button
                ...
                className={`px-5 py-2 text-sm font-bold transition-all rounded-lg border-2 ${selectedRole === role ? "bg-white text-black border-white shadow-lg" : "bg-zinc-900/40 text-zinc-400 border-zinc-800 hover:border-zinc-500"}`}
              >
                {role}
              </button>
            ))}
              ...
              {filteredHeroes.map((hero) => (
                ...
                >
                  <div className="relative w-24 h-24 mb-4 rounded-2xl border-2 border-zinc-900 bg-zinc-900/60 transition-all group-hover:border-white overflow-hidden shrink-0 shadow-xl">
                    <img
                      src={hero.iconUrl || "/placeholder-hero.png"} // FIX 2: Use a fallback or null check
                      alt={hero.hero_name}
                      ...
                      {...(hero.iconUrl ? {} : { "aria-hidden": true })} // Accessibility
                    />
                  </div>
